...
class HomeRepository extends IHomeRepository {
  ...
  @override
  Future<CommonResponse> shortListUser(
      {required String profileId, required String create}) async {
    try {
      log("reaching here");
      final response = await client
          .getWithProfile(
  ...
  @override
  Future<CommonResponse> acceptRejectIntrest(
      {required String senderId, required String status}) async {
    try {
      final response = await client
          .getWithProfile(
            url:
                "${AppUrls.acceptRejectUrl}state=$status&sent_user_id=$senderId",
          )
          .timeout(
            const Duration(seconds: 30),
          );
  ...
  Future<CommonResponse> undoIntrest({required String reciverID}) async {
    try {
      final response = await client.postWithProfile(
          url: "${AppUrls.undoIntrestUrl}$reciverID/", body: {}).timeout(
        const Duration(seconds: 30),
      );
      final decode = jsonDecode(response.body) as Map<String, dynamic>;
      return CommonResponse.fromJson(decode);
