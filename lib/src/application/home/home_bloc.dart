...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  FutureOr<void> _undoIntrest(
      UndoIntrestEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
      ...
      final res =
          await _iHomeRepository.undoIntrest(reciverID: event.recieverID);
      ...
      if (res.object == 1) {
        ...
        final int userIndexFromMatchList = state.allMatchList.indexWhere(
            (user) => user.id.toString() == event.recieverID.toString());
        final int userIndexFromSugessionsList = state.sugesstionsList
            .indexWhere(
                (user) => user.id.toString() == event.recieverID.toString());
        final int userIndexFromNewList = state.newMatchList.indexWhere(
            (user) => user.id.toString() == event.recieverID.toString());

        CustomerData profileData = state.profileData;
        if (profileData.id != null) {
          profileData =
              profileData.copyWith(interestStatus: const InterestStatus());
        }
        allMatchList[userIndexFromMatchList] =
            allMatchList[userIndexFromMatchList]
                .copyWith(interestStatus: const InterestStatus());
        allSugetionsList[userIndexFromSugessionsList] =
            allSugetionsList[userIndexFromSugessionsList]
                .copyWith(interestStatus: const InterestStatus());
        allNewList[userIndexFromNewList] = allMatchList[userIndexFromNewList]
            .copyWith(interestStatus: const InterestStatus());

        emit(
          state.copyWith(
              intrestStatus: StatusSuccess(),
              allMatchList: allMatchList,
              sugesstionsList: allSugetionsList,
              newMatchList: allNewList,
              profileData: profileData),
        );
      } else {
        emit(state.copyWith(
          intrestStatus:
              StatusFailure("${res.message ?? ""} object : ${res.object}"),
        ));
      }
  ...
  FutureOr<void> _acceptRejectIntrest(
      AcceptRejectIntrestEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
          intrestStatus: const StatusInitial(),
      ...
      final res = await _iHomeRepository.acceptRejectIntrest(
          senderId: event.senderID, status: event.status);
      ...
      if (res.respCode == 1) {
        ...
        final int userIndexFromMatchList = state.allMatchList.indexWhere(
            (user) => user.id.toString() == event.senderID.toString());
        final int userIndexFromSugessionsList = state.sugesstionsList
            .indexWhere(
                (user) => user.id.toString() == event.senderID.toString());
        final int userIndexFromNewList = state.newMatchList.indexWhere(
            (user) => user.id.toString() == event.senderID.toString());
        final InterestStatus interestStatus = InterestStatus(
            interestStatus: event.status,
            reciever: int.parse(event.currentUserID),
            sender: int.parse(event.senderID));
        CustomerData profileData = state.profileData;
        if (profileData.id != null) {
        ...
        allMatchList[userIndexFromMatchList] =
            allMatchList[userIndexFromMatchList]
                .copyWith(interestStatus: interestStatus);
        allSugetionsList[userIndexFromSugessionsList] =
            allSugetionsList[userIndexFromSugessionsList]
                .copyWith(interestStatus: interestStatus);
        allNewList[userIndexFromNewList] = allMatchList[userIndexFromNewList]
  ...
  FutureOr<void> _createIntrest(
      CreateIntrestEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
          intrestStatus: const StatusInitial(),
      ...
      final res =
          await _iHomeRepository.sendIntrest(reciverID: event.reciverId);
      ...
      if (res.respCode == 1) {
        ...
        final int userIndexFromMatchList = state.allMatchList.indexWhere(
            (user) => user.id.toString() == event.reciverId.toString());
        final int userIndexFromSugessionsList = state.sugesstionsList
            .indexWhere(
                (user) => user.id.toString() == event.reciverId.toString());
        final int userIndexFromNewList = state.newMatchList.indexWhere(
        ...
        final InterestStatus interestStatus = InterestStatus(
            ...
            reciever: int.parse(event.reciverId),
            sender: int.parse(event.senderID));
        CustomerData profileData = state.profileData;
  ...
  FutureOr<void> _loadProfile(
      LoadProfileEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
            loadProfileStatus: const StatusInitial(),
      ...
      final res =
          await _iHomeRepository.loadProfile(profileId: event.profileId);
      if (res.respCode == 1) {
        emit(state.copyWith(
  ...
  FutureOr<void> _loadSugessionsMatches(
      LoadAllSugetionsEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
            sugesstionsListStatus: const StatusInitial(),
            sugesstionsList: const []),
      );
  ...
  FutureOr<void> _loadNewMatches(
      LoadNewMatchesEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
            newMatchStatus: const StatusInitial(), newMatchList: const []),
      );
  ...
  FutureOr<void> _loadMatches(
      LoadAllMatchesEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
            allMatchStatus: const StatusInitial(), allMatchList: const []),
      );
  ...
  FutureOr<void> _shortlistUser(
      ShortListEvent event, Emitter<HomeState> emit) async {
    try {
      emit(
        state.copyWith(
      ...
      final res = await _iHomeRepository.shortListUser(
          profileId: event.profileId, create: event.create);
      emit(state.copyWith(
      ...
      if (res.respCode == 1) {
        ...
        if (state.profileData.id != null) {
          ...
          profileData = profileData.copyWith(
              shortListed: (profileData.shortListed ?? false) ? false : true);
        }

        final int userIndexFromMatchList = state.allMatchList.indexWhere(
            (user) => user.id.toString() == event.profileId.toString());
        final int userIndexFromSugessionsList = state.sugesstionsList
            .indexWhere(
                (user) => user.id.toString() == event.profileId.toString());
        final int userIndexFromNewList = state.newMatchList.indexWhere(
        ...
        if (userIndexFromMatchList != -1) {
          ...
          allMatchList[userIndexFromMatchList] = user.copyWith(
              shortListed:
                  (state.allMatchList[userIndexFromMatchList].shortListed ??
                          false)
                      ? false
                      : true);
        }
        if (userIndexFromSugessionsList != -1) {
          ...
          allSugetionsList[userIndexFromSugessionsList] = user.copyWith(
              shortListed: (state.sugesstionsList[userIndexFromSugessionsList]
                          .shortListed ??
                      false)
                  ? false
                  : true);
        ...
        if (userIndexFromNewList != -1) {
          ...
          allNewList[userIndexFromNewList] = user.copyWith(
              shortListed:
                  (state.newMatchList[userIndexFromNewList].shortListed ??
                          false)
                      ? false
                      : true);
        }
        ...
        emit(state.copyWith(
            ...
            sugesstionsList: allSugetionsList,
            newMatchList: allNewList,
            refreshShortListItem: StatusSuccess(),
            shortListUserStatus: StatusSuccess(),
            commonResponse: res,
            profileData: profileData));
      } else {
        emit(state.copyWith(
